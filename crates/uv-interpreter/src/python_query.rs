...
fn find_python(
    ...
    if let Some(python_overwrite) = env::var_os("UV_TEST_PYTHON_PATH") {
        for name in selector.possible_names().iter().flatten() {
            for path in which::which_in_global(&**name, Some(&python_overwrite))
                .into_iter()
                .flatten()
            {
                #[cfg(windows)]
                if crate::python_query::windows::is_windows_store_shim(&path) {
                    continue;
                }
